...
The server parses dates and budgets **deterministically before you see them**. Results appear as
canonical strings in the QUALIFICATION STATE block.
...
> did it. Your job below is judgment calls the parser can't make on its own: when to ask instead
> of guessing, and how to phrase dates/money back to the customer.
...
occurrence (already handled by the parser).
