# 05 — Customer Conditions, Tone & Diagnosis

*How* to ask — the counterpart to `06-property-type-playbooks.md` (*what* to ask).
Covers reading the customer's state, resolving ambiguity, and adapting tone.
Merges the former docs 12 (conditions/tone) + 14 (intent diagnosis).
...
Read the building type and transaction type out of every message yourself. Stay aware of the
**currently-detected pair** and keep the conversation anchored to it.

**Detection traps — decide these yourself:**
...
(doc 03 §Q2c). ⛔ Never answer a typo'd area with "area itu tidak ada."
...
(doc 03 §1 Gate B). This line previously listed budget and date as "MANDATORY — never skip";
...
> `04-answer-completeness-and-reask.md`.
...
**A change is granular, never a Q1 wipe (M124/M154) — full rules & table → doc 02 §5.** Changing
...
- `03-qualification-flow.md` — the question sequence and state block
- `04-answer-completeness-and-reask.md` — when an answer doesn't count yet
- `06-property-type-playbooks.md` — what to ask per type
- `08-offtopic-and-escalation.md` — the full off-topic guard
