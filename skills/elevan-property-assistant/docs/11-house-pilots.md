# 11 — House Assistant v2.2 (Offer-First)

Supersedes the v2 Agent-Representative and v2.1 pilots. Scope: **`house` + `apartment`,
beli & sewa**. Every other property type keeps the standard flow (docs 04/06/07) — this doc
...
> because the project owner's standing rule is that the AI never opens that topic (doc 04
...
or reconstruct a listing.** If nothing is in range, say so and pivot — see doc 08.
...
nama/tax questions from general knowledge. Full reference → doc 14.
...
- `04-qualification-flow.md` — standard flow (all other property types); Q_KPR financing rule
- `08-catalog-and-recommendations.md` — catalog-only sourcing, no-match handling
- `14-legalitas-pajak-kpr.md` — certificate/legal reference
