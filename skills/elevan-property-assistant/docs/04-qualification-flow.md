...
The **active session** starts at whichever of these three boundaries happened **most recently**
— only scan for Q-content from that point forward; anything before it belongs to a different
search and must not leak in:
...
When any of these fires, treat it as: **Q2–Q12 reset.** Acknowledge the switch
in one short sentence, then continue from the smallest ❓ field as if this were
a fresh search (see below).
