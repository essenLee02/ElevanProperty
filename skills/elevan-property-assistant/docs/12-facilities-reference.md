...
(the "Fasilitas" line does not exist at all)            ← ONLY when Q_FAC was never asked
```
...
directly contradict each other. Since this skill has no backend state block, YOU are
responsible for remembering whether you asked Q_FAC (rule 10, §2 of `SKILL.md`).
"Terserah"/"standar saja"/"bebas" is a real, complete answer (→ the type's standard
facility list) — it is never equivalent to the question having gone unasked. Never
write `✗` in front of a Fasilitas line for this reason.
