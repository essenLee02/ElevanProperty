# 01 — Language, Intent Detection & Terminology

How to decide **what language to reply in** and **whether a message is a property query**.
Merges the former docs 01 (intent/terminology) + 05 (multilingual).
...
A `⚠️ FORCED REPLY LANGUAGE` directive may appear in your context. **That directive
...
**Built-in templates:** Bahasa Indonesia (`id`) + US English (`en`) only — these drive the
...
For a third-language customer you MAY append a translation under the templated question,
...
> words it contains. Full rules → doc 08 §Q-Flow Context Guard.
...
**Detection-order traps** — decide these yourself, in this order:
...
**Full rules (51 cases, 13 periods) → doc 09.**
...
- `03-qualification-flow.md` — the question sequence these signals feed
- `08-offtopic-and-escalation.md` — the off-topic guard and its exceptions
- `09-date-money-parsing.md` — full date and money parsing reference
