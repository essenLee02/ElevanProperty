...
const { extractQualificationState, countCustomerTurns } = require('../services/aiPromptBuilderService');
...
class LanguageDetector {
  ...
  static detect(message = '', history = []) {
    ...
    //    M204 (sim N8): "Ok thanks." di sesi Indonesia → seluruh penutup & summary berbahasa
    //    Inggris. Pesan ≤3 kata mengikuti bahasa sesi bila riwayat customer jelas Indonesia.
    if (_languageKeywords.looksEnglish(text)) {
      const shortMsg = text.trim().split(/\s+/).length <= 3;
      if (shortMsg && Array.isArray(history) && history.length > 0) {
        const custMsgs = history.filter(h => h.role === 'user' || h.role === 'customer').slice(-8).map(h => this.#normalize(h.message || ''));
        const idCount = custMsgs.filter(m => _languageKeywords.indonesianHits(m, this.#INDONESIAN_WORDS).length).length;
        const enCount = custMsgs.filter(m => _languageKeywords.looksEnglish(m)).length;
...
class ResponseBuilderWhatsApp {
  ...
  #catalogListWhatsApp(properties = [], lang = 'en', limit = 6, startIndex = 0) {
    return properties.slice(0, limit)
      .map((item, i) => this.#catalogItemWhatsApp(item, startIndex + i, lang))
      .join('\n\n');
  }
   ...
   */
  // M197 (13 Sep 2026): `startIndex` = jumlah kartu yang SUDAH terkirim di sesi —
  // listing tambahan dinomori berlanjut (3, 4, 5…), bukan mulai dari 1 lagi,
  // supaya "saya pilih no 3" menunjuk kartu yang benar (produksi 13 Sep: kartu
  // tambahan dinomori 1 lagi → "belum ada no. 3").
  renderListingCards(properties = [], lang = 'en', limit = 6, startIndex = 0) {
    return this.#catalogListWhatsApp(properties, lang, limit, startIndex);
  }
  ...
  agentBrief(brief) {
    ...
    // M193 — Q4 (penghuni) DITANYA dan DIJAWAB tapi tidak pernah dirender di
    // sini (jalur LLM sudah punya barisnya). Tanpa ini jawaban "bersama
    // keluarga" hilang dari ringkasan — atau, lebih buruk, nyasar ke baris
...
class ConversationQualifier {
  ...
  /** M198 — kalimat customer yang benar-benar PERNYATAAN hal yang dihindari (bukan pertanyaan/permintaan percakapan). */
  static #redFlagStatements(history = [], userMessage = '') {
    const msgs = [...(Array.isArray(history) ? history : []).filter(h => h.role === 'user' || h.role === 'customer').map(h => String(h.message || '')), String(userMessage || '')];
    const AVOID = /\b(tidak|tdk|ga|gak|gk|nggak|ngga|enggak|jangan|hindari|dihindari|anti|bukan|tanpa|jauh\s+dari|hadap\s+barat|gang\s+sempit|rumah\s+tua|rawan|bising|banjir|macet|kumuh|panas|bau|kurang\s+(suka|cocok)|tidak\s+cocok)\b/i;
    const CONVO = /\b(?:jangan|tidak\s+usah|nggak\s+usah|gak\s+usah|nggak\s+mau|gak\s+mau|tidak\s+mau|belum\s+mau)\b[^.?!]{0,25}\b(?:kirim|tanya|nanya|ulang|listing|survei|survey|viewing|katalog|bahas|kpr|ditanya|dipaksa|lihat-lihat|ketemu\w*)\b|\bselain\s+\w+(?:\s+\w+)?\s+(?:jangan|nggak|gak|tidak)\b|\bmaunya\s+(?:dekat|deket|di)\b/i;
    return msgs
      .map(m => m.trim())
      .filter(m => m && !/\?/.test(m) && AVOID.test(m) && !CONVO.test(m))
      .join('. ')
      .toLowerCase();
  }
  ...
  static buildProfile(history = [], userMessage = '', filters = {}) {
    ...
    const viewingDayRefResolved = (() => {
      try {
         ...
         * mau dicegah di sini. */
        if (!scoped) {
          /* M193 — PASANGKAN tiap pesan AI yang menyinggung survei/viewing dengan
           * JAWABAN LANGSUNGNYA saja, bukan "semua pesan sesudah kata survei
           * pertama". Simulasi: tawaran "Mau saya jadwalkan survei ke unit ini?"
           * membuka jendela itu, lalu "Rencana bln 2 dpn" — jawaban atas
           * pertanyaan MASUK dua giliran kemudian — ikut terparsing jadi tanggal
          ...
          for (let i = 0; i < activeHistory.length; i++) {
            const r = activeHistory[i];
            if (!(r && (r.role === 'ai' || r.role === 'assistant') && AI_VIEWING_Q_RE.test(String(r.message || '')))) continue;
            const next = activeHistory[i + 1];
            if (next && (next.role === 'user' || next.role === 'customer')) answers.push(String(next.message || ''));
            else if (i === activeHistory.length - 1) answers.push(String(userMessage || ''));
          }
          if (answers.length) scoped = answers.join('. ').trim();
        }
    ...
    const moveInScoped = (() => {
      ...
      /* M193 — deteksi tanggal lewat PARSER RESMI, bukan daftar regex sendiri.
       * Daftar lama tidak kenal "bln 2 dpn" / "2 bulan lagi" / "akhir tahun"
       * dan tidak membuka singkatan, sehingga jawaban Q8 yang DITANYA dan
       * DIJAWAB ("Rencana bln 2 dpn") hilang dari ringkasan (transkrip 14 Sep
       * 2026). parseCustomerDate + expandAbbreviations sudah menangani semua
       * bentuk itu; daftar keyword ringan tetap dipakai sebagai pelengkap untuk
       * jawaban tanpa tanggal ("segera", "sudah mau"). */
      const hasDate = (t) => {
        if (MONTH_ID_WB.test(t) || MONTH_EN_WB.test(t) || OTHER_DATE.test(t)) return true;
        try {
          const parsed = parseCustomerDate(expandAbbreviations(t), new Date());
          return Boolean(parsed && (parsed.status === 'ok' || parsed.status === 'ask_current_month' || parsed.status === 'ask_soon'));
    ...
    /* M198 (13 Sep 2026) — SATU SUMBER KEBENARAN SLOT: ekstraktor bersama
     * (extractQualificationState). Simulasi 10 sesi: area "Candramas"/"Kalijudan"/
     * "Driyorejo" tidak ada di daftar statis hasDistrict → Q2c ditanya ulang
     * padahal kartu area itu sudah dikirim; "nggak mau bahas KPR dulu" terbaca
     * hasFinancing+financingIsKPR → Q_KPR-a bank/DP tetap ditanya. */
    let qsShared = {};
    // Riwayat PENUH: ekstraktor menangani reset granular sendiri (M124/M132), jadi kota
    // tetap Gresik saat hanya transaksi yang berganti ("sewa saja, tetap di Driyorejo").
    try { qsShared = extractQualificationState(history, userMessage) || {}; } catch (_) { /* fail-open */ }
    const sharedDistrict = Boolean(qsShared.district);
    const financingRefused = Boolean(qsShared.financingRefused);
    // M198: kartu listing SUDAH terkirim di sesi ini → Q2b ("sudah lihat berapa?") tidak relevan.
    const cardsShownBefore = history.some(h => (h.role === 'ai' || h.role === 'assistant') && isCardMessage(String(h.message || '')));
    // Kota yang tercetak di kartu yang sudah dikirim ("📍 Lokasi: GRESIK, JAWA TIMUR")
    // = kota yang sudah disepakati; jangan tanya "di kota mana?" lagi (P3/P8 simulasi).
    const cardCity = (() => {
      for (let i = activeHistory.length - 1; i >= 0; i--) {
        const h = activeHistory[i];
        if (!(h.role === 'ai' || h.role === 'assistant')) continue;
        const m = String(h.message || '').match(/Lokasi\s*:\s*([^,\n]+)/i);
        if (m) return m[1].trim().toLowerCase();
      }
      return '';
    })();

    // M193 — pertanyaan AI TERAKHIR menentukan slot mana yang dijawab customer:
    // "tinggal bersama siapa?" (Q4) vs "langsung bisa jadwalkan atau perlu
    // koordinasi?" (Q9). Jawaban "bersama keluarga" milik yang terakhir ditanya.
    const lastAiAskedDecision = /\b(koordinasi|keluarga lain|langsung bisa jadwalkan|coordinate|decide together|joint decision)\b/i.test(lastAiMsgForViewing);
    ...
    const profile = {
      ...
      // M198: kota dari kartu terkirim / ekstraktor bersama (ganti transaksi "tetap di
      // Driyorejo" memangkas riwayat, tapi kotanya masih Gresik) — jangan tanya ulang.
      location        : filters.location        || cardCity || String(qsShared.city || '') || '',   // 'malang'|'surabaya'|...
      ...
      hasFinancing: financingRefused || this.#has(custText, [
      ...
      financingIsKPR: !financingRefused && this.#has(custText, ['kpr', 'kredit', 'cicil', 'kombinasi', 'kpt', 'mortgage', 'syariah', 'murabahah', 'flpp', 'subsidi']),
      financingRefused,
      ...
      hasKprDetails: financingRefused || this.#has(custText, [
      ...
      hasDistrict: (() => {
        ...
        return sharedDistrict || ALL_AREAS.some(a => custText.toLowerCase().includes(a));
      })(),
      ...
      // M204: Q2b ("Sudah lihat berapa…") hanya bila customer sendiri mengisyaratkan pernah
      // mencari/melihat unit — pembuka yang sudah jelas kebutuhannya (sim N7) jangan diinterogasi.
      customerHintsPriorSearch: /\b(?:sudah|udah|pernah|sempat|lagi|sedang)\s+(?:lihat|liat|cek|survei|survey|banding\w*|nyari|cari)\b|\b(?:belum|kurang|nggak|gak)\s+cocok\b|\bdi\s+(?:tempat|agen|aplikasi)\s+lain\b|\blihat-lihat\b|\bbanding\w*\b/i.test(custText),
    ...
    // just those to the active session.
      ...
      // M198: "sewa saja, tetap di Driyorejo" — kota tidak disebut ulang, tapi ekstraktor
      // bersama (reset granular M124/M132) masih memegangnya; jangan tanya kota lagi.
      profile.location  = activeLocation || String(qsShared.city || '');
  ...
  static getNextQuestion(profile, lang = 'id', priceAnchors = null, mode = 'catalog', agentAreaOptions = null) {
    ...
    if (!profile.hasSearchHistory && !profile.aiAskedSearchHist && !profile.customerStatedSearchHistory && profile.customerHintsPriorSearch && profile.aiCount <= 3 && loc) {
  ...
  static buildAgentBrief(profile, filters = {}, history = [], userMessage = '') {
    ...
    const brief = {
      ...
      buildingType: (() => {
        /* M193 — unit yang DIPILIH menentukan tipe. Transkrip 14 Sep 2026:
         * customer membuka dengan "rumah", lalu memilih kartu "Jambangan
      ...
      ...(() => {
        /* M198 (13 Sep 2026) — sumber Hindari HANYA kalimat PERNYATAAN penghindaran.
         * Simulasi: "Rumahnya banjir nggak kalau hujan?" (pertanyaan) → "Tidak mau
         * banjir"; "nggak mau bahas KPR dulu" (permintaan percakapan) → "Jauh dari
         * bahas kpr dulu". Seluruh custText dulu dipakai bila hasRedFlags true. */
        ...
        // M208 (sim R3): syarat wajib dari ekstraktor ("harus satu lantai, akses kursi roda") ikut ke Prefer.
        if (qualState.preferences) {
          const rfLower = String(redFlagsSource || '').toLowerCase();
          for (const item of String(qualState.preferences).split(/\s*,\s*/).map((x) => x.trim()).filter((x) => x.length >= 3)) {
            // Item yang juga muncul di kalimat penghindaran ("jauh dari pemakaman, masjid") bukan Prefer.
            if (rfLower.includes(item.toLowerCase())) continue;
            const lab = item.charAt(0).toUpperCase() + item.slice(1);
            if (!prefer.some((p) => String(p.label).toLowerCase().includes(item.toLowerCase())) && !avoid.some((a) => String(a.label).toLowerCase().includes(item.toLowerCase()))) prefer.push({ label: lab });
          }
        }
        return {
          avoidItems : avoid,
      ...
      apartmentPref: (() => {
        ...
        // M198: dulu SELURUH custText dipakai bila hasApartmentPrefs true → baris
        // "Tower/Lantai" berisi seluruh percakapan. Kini hanya kalimat PERNYATAAN
        // yang menyebut tower/lantai/hadap — pertanyaan ("lantai berapa?") tidak.
        const prefSentences = (profile.hasApartmentPrefs ? custText : '')
          .split(/[.!?\n]+/).map(t => t.trim())
          .filter(t => t && /\b(tower|lantai|lt\.?\s*\d|hadap|view|menghadap|orientasi)\b/i.test(t) && !/\b(berapa|apa|apakah|gimana)\b/i.test(t))
          .join('. ');
        const raw = qualState.apartmentPref || prefSentences;
  ...
  static #extractMoveInDate(custText) {
    /* M193 — PARSER RESMI DULU. Fungsi ini kembaran ketiga dari
     * parseCustomerDate (setelah viewingDayRef M189f dan moveInScoped M193):
     * tidak kenal "2 bulan lagi"/"bln 2 dpn"/"akhir tahun", dan mengembalikan
     * nilai RELATIF ("Bulan depan") — melanggar aturan ringkasan "tanggal
     * wajib absolut". Delegasikan dulu; regex lama tinggal cadangan. */
    try {
      const parsed = parseCustomerDate(expandAbbreviations(String(custText || '')), new Date());
      if (parsed && parsed.status === 'ok' && parsed.formatted) return parsed.formatted;
      if (parsed && parsed.status === 'ask_soon') return 'Secepatnya (urgent)';
    } catch (_e) { /* jatuh ke regex lama */ }
  ...
  static #extractDecisionMaker(custText, profile) {
    /* M193 (14 Sep 2026) — "BERSAMA KELUARGA" ADALAH JAWABAN PENGHUNI (Q4),
     * BUKAN PENGAMBIL KEPUTUSAN (Q9). Transkrip produksi: AI bertanya "Nanti
     * akan tinggal bersama siapa saja?", customer menjawab "Bersama keluarga;
     * Kak" — dan ringkasan menulis `✓ Keputusan bersama: Bersama keluarga`.
     * Dua sebab: (1) /sama keluarga/ tanpa \b cocok di dalam "berSAMA KELUARGA";
     * (2) frasa "sama suami/istri/keluarga/teman" dianggap sinyal keputusan
     * padahal itu persis kata-kata orang menjawab "tinggal sama siapa".
     * Aturan: frasa co-habitant hanya bernilai keputusan bila ada ISYARAT
     * KEPUTUSAN (koordinasi/diskusi/izin/persetujuan/keputusan) atau AI memang
     * baru menanyakan hal itu (lastAiAskedDecision). Selain itu -> UNKNOWN,
     * dan Q4 (#extractHouseholdSummary) yang mencatatnya. */
    const decisionCue = /\b(koordinasi\w*|diskusi\w*|izin|persetujuan|keputusan\w*|memutuskan|putuskan|decide|discuss|check with|tanya\s+(?:dulu\s+)?(?:ke|sama)\s)\b/i.test(custText);
    const explicitOk  = decisionCue || Boolean(profile && profile.lastAiAskedDecision);

    if (/\b(sendiri|alone|solo)\b/.test(custText) && explicitOk)                    return 'Sendirian';
    if (/\b(sama|bersama|dengan)\s+suami\b|with husband/.test(custText) && explicitOk)   return 'Bersama suami';
    if (/\b(sama|bersama|dengan)\s+istri\b|with wife/.test(custText) && explicitOk)      return 'Bersama istri';
    if (/\b(sama|bersama|dengan)\s+pasangan\b|with partner/.test(custText) && explicitOk) return 'Bersama pasangan';
    if (/\b(sama|bersama|dengan)\s+keluarga\b|with family/.test(custText) && explicitOk)  return 'Bersama keluarga';
    if (/\b(sama|bersama|dengan)\s+teman(?:-teman)?\b|with friends?/.test(custText) && explicitOk) return 'Teman';
    if (/\b(langsung bisa|bisa langsung)\b/.test(custText))     return 'Solo (bisa langsung jadwalkan)';
    if (/\bkoordinasi\w*|perlu diskusi\b/.test(custText))         return 'Perlu koordinasi (joint decision)';
  ...
  static #extractLeaseDuration(custText) {
    ...
    // M204: "Cicilan 15 tahun berapa per bulan?" / "tenor 20 tahun" = pembiayaan, bukan durasi sewa (sim N2).
    const src = String(custText || '').replace(/\b(?:cicil\w*|angsur\w*|tenor|kpr|kredit)\b[^.?!\n]{0,25}?\d+\s*(?:tahun|thn|bulan|year|month)s?\b/gi, ' ');
    const m = src.match(/(\d+)\s*(hari|malam|minggu|bulan|tahun|day|night|week|month|year)s?\b/i);
    if (m) {
      const unitMap = {
  ...
  static #extractViewingPreference(custText, profile = {}) {
    const wantsCatalogOnly =
      ...
      /(minta|kasih|kirim|send)\s*(list|listing|katalog|daftar)/i.test(custText) ||
      /(list|listing|katalog|daftar)\s*(aja|saja|only|dulu)/i.test(custText);
    /* M204 (18 Sep 2026) — "sy cuma mau lihat pilihan dulu" (frustrasi, K5 C7) lalu
     * "Survei sabtu ini bisa? Pagi." → summary 'Viewing: Minta listing'. Tanggal survei
     * konkret yang disebut KEMUDIAN menang; penolakan hanya berlaku bila tidak ada jadwal. */
    if (wantsCatalogOnly && !profile.viewingDayRef && !profile.viewingDeferred) return 'Minta listing';
  ...
  static #buildAvoidPreferPairs(redFlagsRawText = '', apartmentPrefRawText = '') {
    ...
    for (const m of String(redFlagsRawText || '').matchAll(FAR_RE)) {
      ...
      for (let raw of m[1].split(/\s*,\s*|\s+dan\s+|\s+atau\s+|\s*&\s*/i)) {
        ...
        // M208: "tidak mau bertingkat" adalah CIRI unit, bukan tempat → "Tidak mau …"; "jauh dari/dekat X" tetap "Jauh dari …".
        const isPlaceAvoid = /\b(?:jauh|dari|dekat|deket)\b/i.test(String(m[0]).slice(0, String(m[0]).length - String(m[1]).length));
        const label = isPlaceAvoid ? `Jauh dari ${item.toLowerCase()}` : `Tidak mau ${item.toLowerCase()}`;
...
class ChatbotPrivateService {
  ...
  static async generateResponseForTerminalMassege({
    ...
    {
      /* M204 (18 Sep 2026) — kartu dari PENCARIAN LAMA (sebelum ganti kota/tx/tipe) bukan
       * "unit yang sudah dikirim" untuk pencarian sekarang. Sim K3: 1 kartu Gresik terkirim,
       * customer pindah ke Citraland Surabaya (stok sewa kosong), lalu "Can we view it next
       * Wednesday?" → gerbang ini dilewati (anyCardSent=true) dan jatuh ke skrip "other
       * neighbourhoods?" tiga giliran berturut-turut. Kartu dihitung sejak searchStartIdx;
       * kartu lama disebutkan sebagai opsi supaya customer bisa memilih. */
      let searchStartNC = 0;
      try { const { extractQualificationState: _eqsNC } = require('../services/aiPromptBuilderService'); searchStartNC = Number((_eqsNC(history, userMessage) || {}).searchStartIdx) || 0; } catch (_) { searchStartNC = 0; }
      const cardRows = history.slice(searchStartNC);
      const anyCardSent = cardRows.some((h) => /^(ai|assistant)$/i.test(String(h.role || '')) && isCardMessage(String(h.message || '')));
      const oldCardSent = !anyCardSent && history.slice(0, searchStartNC).some((h) => /^(ai|assistant)$/i.test(String(h.role || '')) && isCardMessage(String(h.message || '')));
      const oldCardTitle = oldCardSent ? (() => { try { const { parseAllShownCards: _pac } = require('../utils/listingSelectionGate'); const all = _pac(history.slice(0, searchStartNC)); return all.length ? all[all.length - 1].title : ''; } catch (_) { return ''; } })() : '';
      ...
      // M204 (sim N6): "Survei Rabu depan pagi." (pernyataan jadwal tanpa mau/bisa) juga = minta survei.
      const statesViewingSchedule = /\b(survei|survey|viewing|site\s*visit)\b/i.test(userMessage)
        && (Boolean(parseCustomerDate(userMessage)) || /\b(pagi|siang|sore|malam|jam|pukul|besok|lusa|depan|ini)\b/i.test(userMessage));
      const wantsViewNoUnit = (customerRequestsViewing(userMessage) || statesViewingSchedule) && !customerDeclinesViewing(userMessage);
      /* M204 (sim N4/N5): stok kota/area sudah dinyatakan KOSONG, lalu customer bertanya soal
       * unit yang tidak ada ("Aman nggak buat anak cewek? Ada penjaga?") → jawab jujur,
       * bukan skrip Q8. Pertanyaan istilah/jarak/ketersediaan sudah ditangani sebelumnya. */
      const emptySaidEarly = history.some((h) => /^(ai|assistant)$/i.test(String(h.role || '')) && /belum ada di data saya|not (?:yet )?in my data|memang belum ada|don'?t have any|tetap belum ada|belum punya listing|masih belum ada|belum ada yang sesuai|still nothing|nothing in/i.test(String(h.message || '')));
      ...
      if (!pickSticks && !anyCardSent && !definitionQ && !termFirst && (refersToUnit || wantsViewNoUnit || asksAboutNonexistentUnit || (isAttributeQuestion(userMessage) && !/\b(?:yang|yg|ada)\s+(?:\d|dua|tiga|empat)\s*(?:kamar|kt|lantai)\b|\bada\s+(?:yang|yg)\b/i.test(userMessage)))) {
        ...
        const emptySaid = [...history].reverse().find((h) => /^(ai|assistant)$/i.test(String(h.role || '')) && /belum ada di data saya|not (?:yet )?in my data|memang belum ada|don'?t have any|tetap belum ada|belum punya listing|masih belum ada|belum ada yang sesuai|still nothing|nothing in/i.test(String(h.message || '')));
        const missing = [];
        ...
        const oldNote = oldCardTitle
          ? (lang === 'id' ? ` (Kalau maksudnya unit sebelumnya, *${oldCardTitle}*, sebut saja — bisa saya jadwalkan.)` : ` (If you meant the earlier unit, *${oldCardTitle}*, just say so and I'll schedule it.)`)
          : '';
        const reply = wantsViewNoUnit
          ? (lang === 'id'
            ? `Boleh, Kak 😊 Tapi untuk pencarian ini belum ada unit yang saya kirim untuk disurvei — ${reason} Begitu ada unit yang cocok, jadwal surveinya langsung saya atur.${oldNote}`
            : `Sure 😊 But for this search I haven't sent any unit to view yet — ${reason} As soon as there is a matching unit, I'll set the viewing right away.${oldNote}`)
          : (lang === 'id'
      ...
      /* M204 — "Afternoon, around 2 pm." / "Jam 10 pagi ya" SESUDAH jawaban "belum ada unit
       * untuk disurvei" (sim K3 C10): catat jamnya, jangan lanjut skrip area. */
      /* Sim N10: jadwal yang DICATAT tanpa unit lalu diubah/dibatalkan berulang ("Eh ganti
       * Sabtu aja", "Jam 10 pagi", "batal dulu", "Kalau Minggu depan jam 10 bisa?") — semua
       * dijawab di sini selama belum ada kartu, bukan skrip Q/reoffer. */
      const noUnitSchedCtx = !anyCardSent && lastAiNC && /haven't sent any unit (?:to view )?yet|belum ada unit yang saya kirim|saya catat, Kak 😊 Begitu ada unit|I'll lock that slot in|dibatalkan dulu, Kak — belum ada unit|cancelled for now — no unit|masih belum ada — mau saya carikan|Tinggal sebut area yang mau dicoba|Still nothing there|Just name the area/i.test(String(lastAiNC.message || ''));
      if (noUnitSchedCtx && /\b(batal\w*|cancel|tunda|undur\s+dulu|skip\s+dulu)\b/i.test(userMessage) && !/\b(jangan|nggak|gak|tidak)\s+(?:batal|cancel)\b/i.test(userMessage)) {
        const reply = lang === 'id'
          ? `Baik, Kak 🙏 Jadwalnya dibatalkan dulu, Kak — belum ada unit yang dikunci juga. Kabari saja kalau sudah ada waktu lagi.`
          : `No problem 🙏 The slot is cancelled for now — no unit was locked yet either. Just tell me when you're free again.`;
        return this.#wrap(reply, { skillInfo, provider: 'no_cards_yet_gate/time_cancelled' });
      }
      if (noUnitSchedCtx && String(userMessage).trim().split(/\s+/).length <= 8 && !customerSignalsClosing(userMessage)) {
        const { parseSurveyTime: _pst, parseCustomerDate: _pcd } = require('../utils/customerDateParser');
        const normU = userMessage.replace(/\bminggu\s+(?:saja|aja|deh)\b/i, 'hari minggu');
        const t = _pst(normU, { requireClockWord: false }) || (/\b(pagi|siang|sore|malam|morning|afternoon|evening)\b/i.test(normU) ? normU.match(/\b(pagi|siang|sore|malam|morning|afternoon|evening)\b/i)[1] : '');
        const d = _pcd(normU); const dF = d && d.status === 'ok' ? d.formatted : '';
        if (t || dF) {
          const when = [dF, lang === 'id' ? t : String(t).replace(/^Jam\s+/i, '')].filter(Boolean).join(', ');
          const reply = lang === 'id'
            ? `Siap, *${when}* saya catat, Kak 😊 Begitu ada unit yang cocok, jadwal itu langsung saya pakai. Sementara itu, mau saya carikan area lain yang ada stoknya?`
            : `Noted — *${when}* 😊 As soon as there's a matching unit I'll lock that slot in. Meanwhile, shall I look at another area that has stock?`;
          return this.#wrap(reply, { skillInfo, provider: 'no_cards_yet_gate/time_noted' });
        }
      }
    ...
    // M204 (sim N7): "Apakah WNA bisa punya SHM?" sesudah memilih unit = pertanyaan aturan, bukan data unit.
    const asksOwnershipRule = /\b(wna|warga\s+negara\s+asing|orang\s+asing|foreigner\w*|expat\w*|pt\s+pma|badan\s+usaha)\b/i.test(userMessage);
    const asksAboutPickedUnitDocs = !asksOwnershipRule && (Boolean(readConfirmedPick(history))
    ...
    // dipanggil).
    /* M199 — JARAK/WAKTU TEMPUH KE UNIT YANG DIPILIH ("dari Waru berapa jauh?",
     * "ke tol berapa?", "ke bandara Juanda?"). Antar-KOTA dihitung tabel statis;
     * antar-area / ke landmark TIDAK bisa dihitung presisi (GOOGLE_ENABLED=false) →
     * jujur + alamat unit supaya customer bisa cek sendiri di maps, agent konfirmasi. */
    {
      const { looksLikeDistanceQuestion, tryAnswerDistanceQuery } = require('../services/distanceEstimationService');
      const distanceLike = looksLikeDistanceQuestion(userMessage)
        || /\bke\s+(?:tol|bandara|stasiun|mall|sekolah|kampus|pusat\s+kota|terminal|pelabuhan)\b|\bberapa\s+(?:menit|jam|km|kilo)\b/i.test(userMessage);
      const pickForDist = distanceLike ? readConfirmedPick(history) : null;
      if (pickForDist) {
        const { parseAllShownCards } = require('../utils/listingSelectionGate');
        const allD = parseAllShownCards(history);
        ...
        const reply = est
          ...
          : (lang === 'id'
            ? `Untuk jarak/waktu tempuh yang persis ke ${unitLine} saya belum bisa hitung, Kak 🙏 Alamatnya bisa dicek di maps, dan agent kami akan bantu konfirmasi rute saat survei.`
            : `I can't compute the exact distance/travel time to ${unitLine} 🙏 The address can be checked on maps, and our agent will confirm the route at the viewing.`);
     ...
     */
    try {
      // M198: pergantian kota/transaksi/tipe membatalkan pilihan & alur pasca-pilihan (paritas M194).
      const qsChange = extractQualificationState(history, userMessage) || {};
      /* M199 — CUSTOMER PLIN-PLAN: "balik ke Pakuwon, yang tadi saya pilih" /
       * "nggak jadi, tetap beli rumah Pakuwon yang nomor 1 tadi" / "yang mana ya?
      ...
      if (recalled) {
        ...
        return this.#wrap(recalled.reply, { skillInfo, filters, provider: 'pick_recall_gate' });
      }
      /* M199 — PERGANTIAN AREA (Pakuwon → Wiyung) = set kartu baru dari nomor 1
       * (kontrak M192), dan gerbang pilihan/survei lama tidak boleh bicara. Area
       * "sebelumnya" = area kartu yang terakhir dikirim; area "sekarang" = sebutan
       * di pesan ini (district ekstraktor mengikuti sebutan terakhir, M199). */
      const areaChangedFromHistory = (() => {
        try {
          const sentAreas = [...(listSentCardsAll(history).areas || [])];   // Set → urutan kirim
          const prevArea = lastSentAreaLabel(history);   // area yang DICARI terakhir (judul blok)
          const curArea = String(detectAreaName(userMessage) || '').toLowerCase();   // area saja, bukan landmark
          if (!prevArea || !curArea || /\b(patokan|dekat|deket|near|sekitar)\b/i.test(userMessage)) return false;
          if (/\b(kerja|kantor|kuliah|sekolah|tinggal|rumah\s+saya|asal|berangkat|dari)\b[^.?!]{0,12}\bdi\b|\bdari\s+[A-Za-z]/i.test(userMessage)) return false;
          // "Pakuwon" ≈ "Pakuwon City" (kartu memakai nama area penuh)
          const same = curArea === prevArea || prevArea.startsWith(curArea + ' ') || curArea.startsWith(prevArea + ' ');
          if (same || isKnownLocationName(curArea)) return false;
          // Kembali ke area yang kartunya SUDAH terkirim sambil merujuk unit/survei
          // ("mau survei ke rumah Pakuwon itu, Sabtu depan") = konteks lama, bukan pencarian baru.
          const seenBefore = sentAreas.some((a) => a === curArea || a.startsWith(curArea + ' ') || curArea.startsWith(a + ' '));
          if (seenBefore && (customerRequestsViewing(userMessage) || /\b(itu|tersebut|tadi|yang\s+saya\s+pilih)\b/i.test(userMessage))) return false;
          return true;
      ...
      if (pick && pick.attributeQuestion && !compareCueEarly) {
        ...
        if (ans) return this.#wrap(ans.reply, { skillInfo, filters, provider: 'card_attribute_gate' });
      } else if (pick) {
      ...
      const viewingConfirm = changeTurn ? null : tryPendingViewingConfirmation({ message: userMessage, history, isId: lang === 'id' });
      if (viewingConfirm) {
      ...
      const viewingSchedule = changeTurn ? null : tryPendingViewingSchedule({ message: userMessage, history, isId: lang === 'id' });
      if (viewingSchedule) {
      ...
      // M199: pertanyaan PEMBANDING ("bedanya", "masing-masing", "lebih murah") ditangani
      // gerbang lanjutan (semua unit yang dipilih), bukan jaring pasca-pilihan (satu unit).
      const compareCue = /\b(bedanya|perbedaan|banding\w*|masing[-\s]?masing|lebih\s+(?:murah|mahal|luas|besar|kecil)|termurah|termahal|paling\s+(?:murah|mahal|luas))\b/i.test(userMessage);
      const postPickFallback = (changeTurn || compareCue) ? null : await tryPostPickFallback({ history, isId: lang === 'id', message: userMessage, userId: agentUserId });
      if (postPickFallback) {
        console.log(`[PrivateAgent] 🎯 Gerbang jaring pengaman pasca-pilihan: ${postPickFallback.verdict}`);
        return this.#wrap(postPickFallback.reply, { skillInfo, filters, provider: postPickFallback.verdict === 'post-pick-attribute' ? 'card_attribute_gate' : 'post_pick_fallback_gate' });
      }
      ...
      // M198: sinyal penutup KAPAN PUN sesudah kartu terkirim (bukan hanya tepat
      // setelah "Ada yang menarik?") → summary sekali; sesudah summary ditangani
      ...
      if (!summarySentBefore && !cardsSentBefore && customerSignalsClosing(userMessage) && !changeTurn) {
        ...
        // Sesi panjang dengan slot lengkap (sim K3: 12 pesan, budget/tanggal/kamar) tetap dirangkum untuk agent.
        if (!slotsC || (emptySaidC && custCountC <= 4)) {
          const reply = lang === 'id'
            ...
            : `You're welcome 🙏 Sorry I couldn't help this time. If you'd like to try another area or type later, just message me 😊`;
          return this.#wrap(reply, { skillInfo, filters, provider: 'closing_without_cards' });
        }
        // Slot lengkap tapi stok kosong & sesi sudah panjang → lead tetap berharga: kirim summary.
        if (slotsC && emptySaidC) {
          const closingProfileE = ConversationQualifier.buildProfile(history, userMessage, filters);
          const closingBriefE = ConversationQualifier.buildAgentBrief(closingProfileE, filters, history, userMessage);
          return this.#wrap(builder.agentBrief(closingBriefE), { skillInfo, filters, provider: 'closing_signal_gate', responseMode: 'summary' });
        }
      }
      if (!summarySentBefore && !changeTurn
         ...
         * WAJIB. Transkrip 14 Sep 2026 yang dipuji pemilik proyek: sewa, Q8
         * (tanggal masuk) belum ditanya, baru ~6 percakapan -> AI lanjut
         * bertanya "Rencananya masuk bulan apa?" alih-alih menutup. Aturannya:
         * terima kasih SAJA + sewa + Q8 belum ditanya + < 10 percakapan ->
         * biarkan alur normal bertanya. Penutup keras ("cukup", "itu saja")
         * tetap langsung ringkasan. */
      ...
      /* ── M198 (13 Sep 2026) — SETELAH SUMMARY, PERCAKAPAN TETAP HIDUP ──────
       * Aturan pemilik proyek: "Walaupun AI sudah memberikan summary, AI masih
       * bisa respon." Simulasi 10 sesi: setiap pesan sesudah summary ("Kalau
       * nego bisa berapa?", "Saya nggak mau bahas KPR", "Rumahnya banjir
       * nggak?", "Budget saya 450 juta", "Oke itu saja dulu") dijawab sapaan
       * Q1 "Halo! Saya siap bantu… sewa atau beli?" — Phase 0 memotong riwayat
       * di summary dan memulai sesi baru. Kini: tanpa pergantian kota/transaksi/
       * tipe dan tanpa niat pencarian baru, pesan sesudah summary dijawab
       * sebagai lanjutan (atribut unit, nego, KPR, budget, survei, penutup).
       * Permintaan listing tetap diteruskan ke gerbang area di bawah. */
      if (!changeTurn) {
    ...
    try {
      if (agentUserId) {
        ...
        /* M198 (13 Sep 2026) — PATOKAN BUKAN AREA (paritas dengan whatsappAIService
         * M191): "Patokannya dekat Pakuwon Mall" / "dekat kampus ITS" tidak boleh
         * menjadi kandidat AREA, atau gerbang menjawab "belum ada di Pakuwon Mall". */
        const lmToken = detectLandmark(userMessage);
        // M198b: bila customer sekaligus MINTA listing ("ada apartemen dekat ITS?"),
        ...
        if (availCity && (txDb || typeDb)) {
          ...
          /* M204 (18 Sep 2026) — "Kalau Mneganti?" / "Kalau di Driyorejo ada yg lebih murah?"
           * saat TIPE-nya memang tidak ada di kota itu (sim K4): dulu paragraf kota yang sama
           * diulang 3x tanpa menyebut area yang customer tanyakan. Sebut areanya (hasil
           * fuzzy typo), jawab singkat, ulangi tipe yang ada. */
          if (cityHit && cityEmptyAlreadySaid && !qs.cityChangedFromHistory && !qs.typeChangedFromHistory && !qs.txChangedFromHistory) {
            let askedArea = '';
            try {
              const { findAreaCandidatesInText: _fac } = require('../services/areaAvailabilityService');
              const am = await _fac({ userId: agentUserId, city: availCity, text: userMessage });
              askedArea = am && am.area ? String(am.area) : '';
            } catch (_) { askedArea = ''; }
            if (askedArea) {
              ...
              const areaNice = askedArea.toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase());
              const reply = lang === 'id'
                ? `Di *${areaNice}* juga belum ada, Kak 🙏 — *${typeWord}* memang belum ada di seluruh *${availCity}* di data saya${have ? ` (yang ada: *${have}*)` : ''}. Mau saya carikan ${have ? `*${have}*` : 'tipe lain'} di ${areaNice}, atau *${typeWord}* di kota lain?`
                : `Nothing in *${areaNice}* either 🙏 — *${typeWord}* isn't in my data anywhere in *${availCity}*${have ? ` (what I have: *${have}*)` : ''}. Shall I look at ${have ? `*${have}*` : 'another type'} in ${areaNice}, or *${typeWord}* in another city?`;
              return this.#wrap(reply, { skillInfo, filters, provider: 'city_availability_gate/area' });
            }
          }
           ...
           * patokan di kota BARU (Q6)" belum punya kode di jalur ini. Simulasi:
           * "Ganti ke Sidoarjo aja" dibalas "Ada target kapan proses belinya?" —
           * area lama (Wiyung) milik kota lama, area kota baru belum ada, dan
           * gerbang kota diam karena Sidoarjo memang ada stoknya. */
          if (qs.cityChangedFromHistory && !availArea) {
            try {
              const cityId = await resolveCityId(availCity);
              const areas = await listAlternativeAreas({ userId: agentUserId, cityId, buildingType: typeDb || undefined, transactionType: txDb || undefined });
              const names = (areas || []).slice(0, 4).map((a) => a.area).filter(Boolean);
              const typeWord = typeRaw ? PropertyFormatter.humanBuildingType(typeRaw, lang) : (lang === 'id' ? 'properti' : 'property');
              const reply = lang === 'id'
                ? `Siap, ganti ke *${availCity}* ya, Kak 😊 Untuk *${typeWord}* di *${availCity}*, area atau patokan mana yang Kakak pertimbangkan?${names.length ? `\nMisalnya ${names.join(', ')}.` : ''}`
                : `Sure, switching to *${availCity}* 😊 For *${typeWord}* in *${availCity}*, which area or landmark are you considering?${names.length ? `\nFor example ${names.join(', ')}.` : ''}`;
              console.log(`[PrivateAgent] 🏙️ Kota berubah -> tanya area di kota baru (${availCity})`);
              return this.#wrap(reply, { skillInfo, filters, provider: 'city_change_area_ask' });
            } catch (cityChangeErr) {
              console.warn('[PrivateAgent] city-change area ask gagal (non-fatal):', cityChangeErr.message);
            }
          }
        }

        /* M198 — GERBANG KETERSEDIAAN HANYA BILA DIMINTA (paritas whatsappAIService
         * M155): (a) customer minta listing/ketersediaan, atau (b) 4 slot lengkap
         * dan kartu BELUM pernah tampil. Simulasi 13 Sep: tanpa syarat ini gerbang
         * menyuntik "2 rumah TAMBAHAN" pada "Kok lama balasnya", "Saya belum tentu
        ...
        const areaChangedAvail = (() => {
          try {
            ...
            const curArea = String(availArea || '').toLowerCase();
            const same = curArea === prevArea || prevArea.startsWith(curArea + ' ') || curArea.startsWith(prevArea + ' ');
            return Boolean(prevArea && curArea && !same && !isAnchorPhrase && detectAreaName(userMessage)
              && !/\b(kerja|kantor|kuliah|sekolah|tinggal|rumah\s+saya|asal|berangkat|dari)\b[^.?!]{0,12}\bdi\b|\bdari\s+[A-Za-z]/i.test(userMessage));
          } catch (_) { return false; }
        })();
        ...
        const budgetNowP = (() => { try { return Boolean(require('../services/propertyRecommendationService').detectBudget(userMessage)); } catch (_) { return false; } })();   // M208 (sim R1)
        const areaEmptyAlreadySaid = Boolean(availArea) && history.some((h) => /^(ai|assistant)$/i.test(String(h.role || ''))
          && !(budgetNowP && /sesuai budget|within (?:that|your) budget|masih belum ada/i.test(String(h.message || '')))
        ...
        /* M204 (18 Sep 2026) — AREA TETAP KOSONG, customer menambah info ("Yg furnished ya,
         * anaknya nggak bawa apa2", sim K1): dulu jatuh ke skrip "Sudah lihat berapa
         * Apartemen di Surabaya?". Paritas dengan reoffer kota di atas: catat, tawarkan
         * ulang area yang ada — hanya pada PERNYATAAN tanpa area/kota baru dan tanpa tanya. */
        if (availArea && txDb && typeDbEff && areaEmptyAlreadySaid && !cardsShownBefore && !changeTurnAvail
            && !/\?/.test(userMessage) && !customerSignalsClosing(userMessage) && !customerAsksAvailability(userMessage)
            && !detectAreaName(userMessage) && !customerRequestsViewing(userMessage) && !customerDeclinesViewing(userMessage)
            && !/\b(?:ya|iya|oke?|boleh|mau|carikan)\b[^.?!]{0,30}\b(?:itu|saja|aja|salah satu|di\s+[A-Z])/i.test(userMessage)
            && !/\b(?:jam|pukul)\s*\d|\b\d{1,2}\s*(?:pagi|siang|sore|malam|am|pm)\b/i.test(userMessage)) {
          try {
            const cityId = await resolveCityId(availCity);
            const areas = await listAlternativeAreas({ userId: agentUserId, cityId, buildingType: typeDbEff || undefined, transactionType: txDb || undefined });
            const names = (areas || []).slice(0, 4).map((a) => a.area).filter(Boolean);
            const areaNice = String(availArea).toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase());
            const reofferAreaSaid = history.some((h) => /^(ai|assistant)$/i.test(String(h.role || '')) && /masih belum ada — mau saya carikan|still nothing there — shall I/i.test(String(h.message || '')));
            // M208 (P7): tawaran ulang cukup SEKALI — sesudah itu info customer diproses alur biasa (jangan 4x "Dicatat ya").
            if (reofferAreaSaid) throw Object.assign(new Error('reoffer-once'), { soft: true });
            const reply = lang === 'id'
              ? (reofferAreaSaid
                ? `Dicatat ya, Kak 📝 Tinggal sebut area yang mau dicoba${names.length ? ` (${names.join(', ')})` : ''}, langsung saya kirim pilihannya.`
                : `Dicatat ya, Kak 📝 Di *${areaNice}* masih belum ada — mau saya carikan di area terdekat yang ada stoknya${names.length ? `, misalnya ${names.join(', ')}` : ''}?`)
              : (reofferAreaSaid
                ? `Noted 📝 Just name the area to try${names.length ? ` (${names.join(', ')})` : ''} and I'll send the options.`
                : `Noted 📝 Still nothing there — shall I look at a nearby area with stock${names.length ? `, e.g. ${names.join(', ')}` : ''}?`);
            return this.#wrap(reply, { skillInfo, filters, provider: 'area_availability_gate/reoffer' });
          } catch (reofferErr) {
            if (!reofferErr.soft) console.warn('[PrivateAgent] area reoffer gagal (non-fatal):', reofferErr.message);
          }
        }
      }
    } catch (availErr) {
   ...
   */
  /**
   * M198 — lanjutan sesudah summary (lihat catatan di pemanggil).
   * @returns {Promise<null|{reply:string, verdict:string}>}
   */
  static async #tryFollowUpAnswer({ message, history = [], lang = 'id', agentUserId = null, qs = {} }) {
    const isId = lang === 'id';
    ...
    const afterSummary = sumIdx >= 0;

    // Niat pencarian BARU (sewa/beli/tipe/kota disebut lagi, atau sapaan+niat) → alur normal.
    const NEW_INTENT_RE = /\b(sewa|menyewa|ngontrak|beli|membeli|rumah|apartemen|apartment|villa|ruko|kos|kantor|gudang|tanah|cari|carikan|nyari)\b/i;
    const GREET_RE = /^[\s.…,!-]*(hi|hai|halo+|hello|hey|pagi|siang|sore|malam|selamat\s+\w+|permisi|assalamualaikum)\b/i;
    const { isBareAvailabilityQuestion: _isBareAvail, parseAllShownCards: _allCards, isAttributeQuestion: _isAttrQ0 } = require('../utils/listingSelectionGate');
    const anyCardsEver = _allCards(rows).length > 0;
    /* M193 — "Masih tersedia?" / "Ada carport?" / "Ada taman?" SETELAH kartu
     * terkirim = pertanyaan tentang unit itu, bukan minta listing baru.
     * Simulasi 12 pesan: dijawab 2 kartu TAMBAHAN, lalu "Ada taman?" sesudah
     * summary bahkan mereset ke "Halo! sewa atau beli?". */
    // M208 (sim R4): "yang kemarin masih tersedia kan?" sesudah pilihan/summary = pertanyaan unit terpilih.
    const asksPickedAvail = anyCardsEver && /\b(?:masih\s+(?:ada|tersedia|available|kosong)|sudah\s+(?:laku|terjual|tersewa))\b/i.test(text) && /\b(?:yang|yg)\s+(?:kemarin|tadi|itu|saya\s+pilih|dipilih)\b|\bunit(?:nya)?\b/i.test(text);
    const asksListings = !asksPickedAvail && ((customerAsksAvailability(text) && !(anyCardsEver && _isBareAvail(text)))
      || Boolean(detectRequestedCount(text)));
    // M208 (sim R4): "Halo, yang kemarin masih tersedia kan?" — sapaan + pertanyaan unit terpilih bukan sesi baru.
    const greetOnly = GREET_RE.test(text) && !asksPickedAvail && !(anyCardsEver && _isAttrQ0(text));
    if (asksListings || greetOnly) return null;
    ...
    if (!shown.length && /\b(?:nomor|nomer|no\.?)\s*\d{1,2}\b/i.test(text) && isAttributeQuestion(text)) {
      return {
        verdict: 'follow-up-no-cards-yet',
        reply: isId
    ...
    if (/\b(lama|lambat|lelet|lemot)\b[^.?!]{0,20}\b(bal[ae]s\w*|respon\w*|jawab\w*|tunggu)\b|\b(dari\s+tadi)\b[^.?!]{0,15}\btunggu|\btunggu\b[^.?!]{0,15}\b(dari\s+tadi|lama)\b/i.test(text)) {
      return {
        ...
        reply: isId
          ...
          : `Sorry for the wait 🙏 I'm here now — anything about the units I sent, or shall I look for more?`,
      };
    }
    const pick = readConfirmedPick(rows);
    const pickedCard = pick
      ? (everShown.find((c) => String(c.title).toLowerCase() === String(pick.title).toLowerCase() && (!pick.priceText || String(c.priceText || '').toLowerCase() === String(pick.priceText).toLowerCase()))
        || everShown.find((c) => String(c.title).toLowerCase() === String(pick.title).toLowerCase()) || null)
      : null;
    // Unit yang BARU SAJA dibahas ("Untuk *Judul* (alamat): …") = rujukan pertanyaan
    // lanjutan tanpa nomor ("Dekat sekolah nggak?", "Nego bisa berapa?").
    /* M193 — RUJUKAN UNIT YANG TAHAN GILIRAN. Versi lama hanya membaca pesan AI
     * TERAKHIR ("Untuk *Judul*…"): begitu ada balasan lain di antaranya
     * (kartu tambahan, klarifikasi), rujukan hilang dan bot mengulang "unit
     * nomor berapa ya?" TIGA KALI berturut-turut (simulasi 12 pesan) padahal
     * customer baru saja membahas "yang no 1". Urutan prioritas sekarang:
     *   nomor di pesan INI > unit yang DIPILIH > nomor terakhir yang customer
     *   sebut (≤6 pesan) > unit terakhir yang dibahas AI (≤10 pesan) > satu-satunya kartu. */
    const byPrintedIndex = (n) => shown.find((c) => c.index === n) || everShown.find((c) => c.index === n) || null;
    const numInCurrent = (() => {
      const m = text.match(/\b(?:yang\s+)?(?:no|nomor|nomer|nomr)\.?\s*(\d{1,2})\b/i) || text.match(/\byang\s+(?:ke-?)?(\d{1,2})\b/i);
      return m ? byPrintedIndex(parseInt(m[1], 10)) : null;
    })();
    const recentCustomerRef = (() => {
      const cust = rows.filter((h) => /^(user|customer)$/i.test(String(h.role || ''))).slice(-6).reverse();
      for (const h of cust) {
        const m = String(h.message || '').match(/\b(?:yang\s+)?(?:no|nomor|nomer|nomr)\.?\s*(\d{1,2})\b/i);
        if (m) { const c = byPrintedIndex(parseInt(m[1], 10)); if (c) return c; }
      }
      return null;
    })();
    const lastDiscussed = (() => {
      const ais = rows.filter((h) => /^(ai|assistant)$/i.test(String(h.role || ''))).slice(-10).reverse();
      for (const h of ais) {
        const m = String(h.message || '').match(/^(?:Untuk|For) \*([^*]+)\*(?:\s*\(([^)]+)\))?/m);
        if (!m) continue;
        const title = m[1].trim().toLowerCase(); const addr = (m[2] || '').trim().toLowerCase();
        const c = everShown.find((x) => String(x.title).toLowerCase() === title && (!addr || String(x.address || '').toLowerCase() === addr))
          || everShown.find((x) => String(x.title).toLowerCase() === title) || null;
        if (c) return c;
      }
      return null;
    })();
    // Anti-loop: klarifikasi "unit nomor berapa?" sudah ditanyakan dan customer
    // TETAP tidak menyebut nomor -> asumsikan rujukan terakhir / unit no. 1, sebut asumsinya.
    const lastAiMsgText = String(([...rows].reverse().find((h) => /^(ai|assistant)$/i.test(String(h.role || ''))) || {}).message || '');
    const clarifierAskedLast = /unit nomor berapa ya|which unit number/i.test(lastAiMsgText);
    let assumedNote = '';
    // M208 (sim R4): customer menyebut ALAMAT ("Rumah di Jl. Rungkut Menanggal No. 38 masih ada?") → kartu itu.
    const addressRef = (() => {
      const cust = rows.filter((h) => /^(user|customer)$/i.test(String(h.role || ''))).slice(-6).reverse();
      for (const h of cust) {
        const m = String(h.message || '').match(/\b(?:jl\.?|jalan)\s+([A-Za-z][\w' .-]{2,40}?)\s*(?:no\.?|nomor)\s*(\d+[A-Za-z]?)\b/i);
        if (!m) continue;
        const street = m[1].trim().toLowerCase(); const num = m[2].toLowerCase();
        const c = everShown.find((x) => { const a = String(x.address || '').toLowerCase(); return a.includes(street) && new RegExp('\\bno\\.?\\s*' + num + '\\b').test(a); });
        if (c) return c;
      }
      return null;
    })();
    let card = numInCurrent || pickedCard || recentCustomerRef || addressRef || lastDiscussed || (shown.length === 1 ? shown[0] : null);
    ...
    //    (Sebelum summary: gerbang sinyal penutup di pemanggil yang mengirim summary.)
    if (afterSummary && customerSignalsClosing(text)) {
      return {
        verdict: 'post-summary-closing',
        reply: isId
          ? `Sama-sama, Kak 🙏 Catatannya sudah saya teruskan ke agent kami. Kalau nanti ada pertanyaan lagi, tinggal chat saja ya 😊`
          : `You're welcome 🙏 Your notes are with our agent. Just message me anytime you have more questions 😊`,
      };
    }
    ...
    if (schedM && !alreadyCancelled) {
      if (/\b(batal\w*|cancel|tunda|undur\s+dulu|skip\s+dulu)\b/i.test(text) && !/\b(jangan|nggak|gak|tidak)\s+(?:batal|cancel)\b/i.test(text)) {
        return {
          verdict: 'follow-up-viewing-cancelled',
          reply: isId
            ? `Baik, Kak 🙏 Survei ${schedM[1]}${schedM[2] ? ` ${schedM[2]}` : ''} dibatalkan dulu — kabari saja kalau sudah ada waktu lagi, nanti dijadwalkan ulang.`
            : `No problem 🙏 The viewing on ${schedM[1]}${schedM[2] ? ` ${schedM[2]}` : ''} is cancelled for now — let me know when you're free again and we'll reschedule.`,
        };
      }
      /* M204 — "Rencana pindah Maret tahun depan" = tanggal MASUK, bukan ubah jadwal survei
       * (sim N2: "Jadwalnya diubah — survei 01 Maret 2027"). Kata "pindah/masuk/check-in"
       * tanpa kata survei = catat sebagai rencana masuk. */
      const monthOrYear = /\b(?:jan(?:uari)?|feb(?:ruari)?|mar(?:et)?|apr(?:il)?|mei|jun[ie]?|jul[iy]?|agu(?:stus)?|aug(?:ust)?|sep(?:tember)?|okt(?:ober)?|oct(?:ober)?|nov(?:ember)?|des(?:ember)?|dec(?:ember)?|tahun\s+(?:depan|ini)|next\s+year|20\d\d)\b/i.test(text);
      const moveInNotResched = (/\b(?:masuk\w*|check[\s-]?in|checkin|menempati|tempati|huni\w*)\b/i.test(text) || (/\bpindah\w*\b/i.test(text) && monthOrYear))
        && !/\b(survei|survey|viewing|lihat|ketemu\w*|jadwal\w*)\b/i.test(text);
      if (moveInNotResched) {
        const { parseCustomerDate: _pcdM } = require('../utils/customerDateParser');
        const dm = _pcdM(text);
        const when = dm && dm.status === 'ok' ? dm.formatted : '';
        return {
          verdict: 'follow-up-move-in-noted',
          reply: isId
            ? `Siap, rencana masuk${when ? ` *${when}*` : 'nya'} saya catat, Kak 😊 Jadwal survei tetap ${schedM[1]}${schedM[2] ? ` ${schedM[2]}` : ''} ya.`
            : `Noted — planned move-in${when ? ` *${when}*` : ''} 😊 Your viewing stays on ${schedM[1]}${schedM[2] ? ` ${schedM[2]}` : ''}.`,
        };
      }
      const reschedCue = /\b(ganti|ubah|pindah|geser|undur|maju\w*|saja|aja|deh|bisa\s*\?|jadi)\b/i.test(text) || /\b(jam|pukul)\s*\d/i.test(text);
      if (reschedCue) {
        const { parseCustomerDate, parseSurveyTime } = require('../utils/customerDateParser');
        const { stripCancelledDayClause: _scd } = require('../utils/listingSelectionGate');
        const norm = _scd(text).replace(/\bminggu\s+(?:saja|aja|deh)\b/i, 'hari minggu');
        const d = parseCustomerDate(norm);
        const newDate = d && d.status === 'ok' ? d.formatted : null;
        const sameTimeKeep = /\bjam\s+(?:yang\s+)?sama\b|\bsame\s+time\b/i.test(text) ? (schedM[2] || null) : null;   // M208 (sim R7)
        const newTime = parseSurveyTime(norm, { requireClockWord: !!newDate }) || (!newDate ? parseSurveyTime(norm, { requireClockWord: false }) : null) || sameTimeKeep;
        if (newDate && newDate === schedM[1] && !newTime) {
          // M208 (sim R4): "Jadi survei besok tetap ya?" — tanggal sama = konfirmasi, bukan perubahan.
          return {
            verdict: 'follow-up-viewing-confirmed',
            reply: isId
              ? `Tetap, Kak 😊 Survei tanggal *${schedM[1]}*${schedM[2] ? `, *${schedM[2]}*` : ''} ya. Nanti tim kami hubungi untuk konfirmasi.`
              : `Yes, still on 😊 The viewing is *${schedM[1]}*${schedM[2] ? `, *${schedM[2]}*` : ''}. Our team will confirm.`,
          };
        }
        if (newDate || newTime) {
    ...
    // Survei sudah DIBATALKAN, lalu customer menyebut jadwal baru ("Kalau Sabtu depan jam 10 bisa?") → jadwalkan ulang.
    if (schedM && alreadyCancelled && !customerDeclinesViewing(text)) {
      const rebook = scheduleViewingFromText(text, isId);
      if (rebook) return { verdict: 'follow-up-viewing-rebooked', reply: rebook.reply };
    }

    // 2--+) Video call / survei online.
    if (/\b(video\s*call|vc|vidcall|virtual\s*tour|survei\s+online|lihat\s+online|live\s+video)\b/i.test(text) && !/\b(nggak|gak|tidak|jangan)\s+(?:mau|usah)\b/i.test(text)) {
      return {
        verdict: 'follow-up-video-call',
        reply: isId
          ? `Bisa, Kak 😊 Video call unitnya bisa diatur lewat agent kami — mau hari dan jam berapa?`
          : `Sure 😊 A video call of the unit can be arranged through our agent — what day and time suit you?`,
      };
    }

    // 2--+) Minta ditelepon / dihubungi agent, atau konfirmasi nomor.
    if (/\b(telepon|telpon|telfon|call|hubungi|kontak|whatsapp|wa)\b[^.?!]{0,30}\b(saya|aku|nomor\s+ini)\b|\b(minta|tolong)\b[^.?!]{0,20}\b(agent\w*|agen\w*)\b[^.?!]{0,20}\b(telepon|telpon|hubungi|call|kontak)\b/i.test(text)) {
      const when = (text.match(/\b(besok\s+(?:pagi|siang|sore|malam)|besok|nanti\s+(?:sore|malam|siang)|hari\s+ini|sekarang|lusa|minggu\s+depan)\b/i) || [])[1];
      return {
        verdict: 'follow-up-callback',
        reply: isId
          ? `Siap, Kak 😊 Saya catat: agent kami akan menghubungi Kakak${when ? ` *${when}*` : ''} di nomor WhatsApp ini.`
          : `Sure 😊 Noted: our agent will contact you${when ? ` *${when}*` : ''} on this WhatsApp number.`,
      };
    }
    ...
    if (/\b(perlu|harus|wajib)\b[^.?!]{0,15}\b(bawa|siapkan|persiap\w*)\b|\bbawa\s+apa\b/i.test(text)) {
      return {
        verdict: 'follow-up-viewing-checklist',
        reply: isId
    ...
    if (/\b(jemput|ketemu\s+di|meeting\s*point|titik\s+temu|di\s+gerbang|di\s+lobby)\b/i.test(text)) {
      return {
        ...
        reply: isId
          ...
          : `Sure 😊 Noted for our agent — the meeting point will be confirmed together with the viewing schedule.`,
      };
    }

    // 2--+) Menunggu agent ("oke nanti saya tunggu dari agentnya").
    if (/\btunggu\b[^.?!]{0,25}\b(agent\w*|agen\w*|kabar\w*|info\w*)\b/i.test(text)) {
      return { verdict: 'follow-up-waiting-agent', reply: isId ? `Siap, Kak 😊 Agent kami akan menghubungi Kakak secepatnya. Kalau ada yang mau ditanyakan lagi, silakan.` : `Sure 😊 Our agent will be in touch shortly. Anything else you'd like to ask, go ahead.` };
    }

    // 2--) Konfirmasi final ("Oke fix yang itu", "deal", "jadi yang itu") atas unit yang sedang dibahas.
    if (card && /\b(fix|deal|oke\s+yang\s+itu|ok\s+yang\s+itu|jadi\s+yang\s+itu|yang\s+itu\s+(?:saja|aja)|ambil\s+yang\s+itu|saya\s+ambil)\b/i.test(text)) {
      return {
        verdict: 'follow-up-confirm-pick',
        reply: isId
          ? `Siap, Kak 😊 Dicatat pilihannya: *${card.title}*${card.priceText ? ` (${card.priceText})` : ''}.${card.address ? `\n\n🏡 Alamat: ${card.address}` : ''}\n\nMau saya jadwalkan survei ke unit ini?`
          : `Great 😊 Your pick: *${card.title}*${card.priceText ? ` (${card.priceText})` : ''}.${card.address ? `\n\n🏡 Address: ${card.address}` : ''}\n\nShall I arrange a viewing for this unit?`,
      };
    }
    ...
    if (/\b(?:nanti|ntar)\b[^.?!]{0,45}\b(?:kabar\w*|hubungi|konfirmasi|infokan)\b|\bbelum\s+(?:bisa|tahu|tau)\s+kapan\b|\bmasih\s+di\s+luar\s+kota\b|\b(?:survei|survey|viewing)\b[^.?!]{0,20}\b(?:nanti|ntar)\s+(?:saja|aja|dulu|dlu)\b/i.test(text)) {
      return {
        ...
        reply: isId
          ...
          : `Sure 😊 I'll wait for your update — whenever you're ready for a viewing, just send me the date.`,
      };
    }
    ...
    if (/\b(?:jangan|jgn|tidak\s+usah|tdk\s+usah|nggak\s+usah|gak\s+usah|ga\s+usah|nggak\s+perlu|gak\s+perlu|tidak\s+perlu|stop)\b[^.?!]{0,20}\b(?:kirim|kasih|tampilkan|tanya|nanya|tanyain|ulang|bahas|survei|survey|listing|katalog|data\s+saya|data)\b/i.test(text)) {
      ...
      return {
        ...
        reply: isId
          ? (aboutViewing ? `Baik, Kak 😊 Survei tidak saya tanyakan lagi — kapan pun Kakak siap, tinggal bilang. Ada yang mau ditanyakan soal unitnya?`
                          : `Siap, Kak 😊 Dicatat — tidak saya kirim ulang. Ada yang mau ditanyakan soal unitnya?`)
          : (aboutViewing ? `Sure 😊 I won't bring up a viewing again — just say the word whenever you're ready. Anything about the units?`
                          : `Noted 😊 I won't resend those. Anything you'd like to know about the units?`),
      };
    }

    // 2+) Peserta survei ("bisa bawa mertua ikut lihat?", "saya sama suami yang datang") — survei sudah/akan dijadwalkan.
    if ((/\b(bawa|ajak|ikut|bareng|sama)\b[^.?!]{0,30}\b(lihat|liat|survei|survey|viewing|ke\s+lokasi|ikut|datang)\b|\b(ikut|bawa|ajak)\b[^.?!]{0,20}\b(mertua|istri|suami|orang\s*tua|keluarga|teman|anak|saudara)\b|\byang\s+(?:datang|ikut)\b|\bdatang\s+(?:sama|bersama|dengan)\b/i.test(text)
         && !customerRequestsViewing(text))
        && !/\b(jangan|tidak|nggak|gak)\b/i.test(text)) {
      return {
        verdict: 'follow-up-participants',
        reply: isId
          ? `Boleh sekali, Kak 😊 Silakan ajak siapa pun yang perlu ikut melihat — saya catat untuk agent kami.`
          : `Of course 😊 Bring anyone who should see it — I've noted it for our agent.`,
      };
    }

    // M208 (sim R10) — TAWARAN HARGA dengan angka ("Kemahalan. 400 juta boleh?", "kalau 1,2 M?")
    // sesudah kartu terkirim → catat angkanya untuk agent, jangan lanjut skrip.
    {
      const offerM = text.match(/(?:^|[^\w])(?:kalau|klo|kl|tawar\w*|nego\w*|kemahalan|mahal|turun|diskon|boleh|bisa)?[^\d]{0,20}?(\d[\d.,]*)\s*(juta|jt|m\b|miliar|milyar|ribu|rb)\b[^.?!]{0,20}?(?:boleh|bisa|dapat|nego|deal|oke|ok|\?)/i);
      const offerCue = /\b(kemahalan|mahal|nego\w*|tawar\w*|turun|diskon|kurang|boleh|bisa)\b/i.test(text) && !/\b(budget|anggaran|dana|sewa|beli|cari|kpr|cicil\w*|dp)\b/i.test(text);
      if (offerM && offerCue && (shown.length || everShown.length)) {
        const amount = `${offerM[1]} ${offerM[2]}`.replace(/\s+m\b/i, ' miliar');
        return {
          verdict: 'follow-up-price-offer',
          reply: isId
            ? `Penawaran *${amount}* saya catat, Kak 📝 Soal harga, keputusan ada di pemilik — agent kami yang akan sampaikan dan konfirmasi ke Kakak. Mau lanjut atur survei sambil menunggu jawabannya?`
            : `Your offer of *${amount}* is noted 📝 The owner decides on price — our agent will pass it on and confirm with you. Shall we set a viewing while we wait?`,
        };
      }
    }
    // M208 (sim R10) — tanggal+jam sukarela sesudah "surveinya kapan pun Kakak siap" = jadwal survei.
    {
      const lastAiOpen = /kapan pun Kakak siap|whenever you're ready|whenever you are ready/i.test(lastAiMsgText);
      if (lastAiOpen && !customerDeclinesViewing(text)) {
        const direct = scheduleViewingFromText(text, isId, { history: rows });
    ...
    if (customerDeclinesViewing(text) || customerIsBrowsing(text)) {
      return {
        verdict: 'follow-up-browsing',
        reply: isId
          ? `Santai saja, Kak 😊 Silakan tanya apa pun soal unitnya kapan saja — surveinya kapan pun Kakak siap.`
          : `No rush 😊 Ask me anything about the units whenever you like — a viewing is ready whenever you are.`,
      };
    }

    // 2) Survei (dengan/tanpa tanggal di pesan yang sama). "Survei Selasa depan jam 3 sore ya"
    //    tanpa kata niat (mau/bisa) tetap = jadwal survei bila tanggalnya terbaca.
    if (!customerRequestsViewing(text) && /\b(survei|survey|viewing|ketemuan|site\s*visit)\b/i.test(text) && !customerDeclinesViewing(text)) {
      const direct = scheduleViewingFromText(text, isId);
      if (direct) return { verdict: 'follow-up-viewing', reply: direct.reply };
    }
    if (customerRequestsViewing(text)) {
      const direct = scheduleViewingFromText(text, isId);
      if (direct) return { verdict: 'post-summary-viewing', reply: direct.reply };
      const scheduled = rows.some((h) => /^(ai|assistant)$/i.test(String(h.role || '')) && /survei (?:dijadwalkan|dicatat) tanggal \*([^*]+)\*/i.test(String(h.message || '')));
      if (scheduled) {
        const last = [...rows].reverse().find((h) => /^(ai|assistant)$/i.test(String(h.role || '')) && /survei (?:dijadwalkan|dicatat) tanggal \*([^*]+)\*/i.test(String(h.message || '')));
        const when = String(last.message).match(/survei (?:dijadwalkan|dicatat) tanggal \*([^*]+)\*(?:,\s*\*([^*]+)\*)?/i);
        return {
          verdict: 'follow-up-viewing-already-set',
          reply: isId
            ? `Sudah, Kak 😊 Surveinya tercatat tanggal *${when[1]}*${when[2] ? `, *${when[2]}*` : ''}. Kalau mau ganti jadwal, sebutkan saja tanggal barunya.`
            : `Already set 😊 Your viewing is on *${when[1]}*${when[2] ? `, *${when[2]}*` : ''}. Tell me a new date if you'd like to change it.`,
        };
      }
      return {
        verdict: 'post-summary-viewing',
        reply: isId ? `Siap, Kak 😊 Enaknya survei tanggal berapa?` : `Great 😊 What date works for the viewing?`,
      };
    }

    // 2b) Minta foto/video/lokasi maps.
    if (/\b(foto\w*|photos?|gambar\w*|video\w*|denah\w*|floor\s*plan|brosur\w*|share\s*lok\w*|maps|lokasinya\s+di\s+maps)\b/i.test(text) && !/\b(video\s*call|vc)\b/i.test(text)) {
      const unit = card ? `*${card.title}*${card.address ? ` (${card.address})` : ''}` : '';
      if (!card && shown.length > 1) {
        return { verdict: 'follow-up-which-card', reply: isId ? `Boleh, Kak 😊 Untuk unit nomor berapa ya (1-${shown.length})?` : `Sure 😊 For which unit number (1-${shown.length})?` };
      }
      return {
        verdict: 'follow-up-media',
        reply: isId
          ? `Siap, Kak 😊 ${/\bvideo/i.test(text) ? 'Video' : /\bdenah|floor/i.test(text) ? 'Denah' : /\bmaps|lok/i.test(text) ? 'Lokasi/maps' : 'Foto'} ${unit || 'unitnya'} saya teruskan ke agent kami untuk dikirimkan ke Kakak ya.`
          : `Sure 😊 I'll have our agent send you the ${/\bvideo/i.test(text) ? 'video' : /\bdenah|floor/i.test(text) ? 'floor plan' : /\bmaps|lok/i.test(text) ? 'location' : 'photos'} for ${unit || 'the unit'}.`,
      };
    }

    // 3) KPR/pembiayaan tidak mau DIBAHAS ("nggak mau bahas KPR dulu") — bukan jawaban
    //    pembiayaan ("bayarnya cash, nggak pakai KPR" = cash → alur Q normal mencatatnya).
    if (/\b(?:nggak|ngga|gak|gk|tidak|tdk|jangan|jgn|males|malas)\b[^.?!]{0,15}\b(?:bahas|tanya|nanya|ngomongin|omongin|singgung)\b[^.?!]{0,15}\b(?:kpr|kpa|kredit|pembiayaan|cicilan|dp)\b/i.test(text)) {
      return {
        verdict: 'post-summary-kpr-refused',
        reply: isId
          ? `Baik, Kak 😊 KPR tidak kita bahas dulu. Ada lagi yang mau ditanyakan soal unitnya?`
          : `Sure 😊 We'll leave financing aside for now. Anything else you'd like to know about the unit?`,
      };
    }

    // 3+) Membandingkan beberapa unit yang pernah dipilih/dibahas ("bedanya apa?",
    //     "luas tanah masing-masing?", "yang lebih murah yang mana?").
    if (/\b(bedanya|perbedaan|banding\w*|masing[-\s]?masing|lebih\s+(?:murah|mahal|luas|besar|kecil)|termurah|termahal|paling\s+(?:murah|mahal|luas))\b/i.test(text)) {
      const { listConfirmedPicks } = require('../utils/listingSelectionGate');
      const picks = listConfirmedPicks(rows).filter((p) => p.card);
      const uniq = [];
      for (const p of picks) if (!uniq.some((u) => u.card.title === p.card.title && u.card.address === p.card.address)) uniq.push(p);
      const { parseAllShownCards } = require('../utils/listingSelectionGate');
      const allCards = parseAllShownCards(rows);
      const set = uniq.length >= 2 ? uniq.map((p) => p.card) : (allCards.length >= 2 ? allCards.slice(-2) : []);
      if (set.length >= 2) {
        if (/\b(lebih\s+(?:murah|mahal)|termurah|termahal|paling\s+(?:murah|mahal))\b/i.test(text)) {
          const priced = set.filter((c) => Number.isFinite(c.priceValue));
          if (priced.length >= 2) {
            const cheaper = /mahal/i.test(text) && !/murah/i.test(text)
              ? priced.reduce((a, b) => (b.priceValue > a.priceValue ? b : a))
              : priced.reduce((a, b) => (b.priceValue < a.priceValue ? b : a));
            const list = priced.map((c) => `• *${c.title}*${c.area ? ` (${c.area})` : ''}: ${c.priceText}`).join('\n');
            return { verdict: 'follow-up-compare-price', reply: isId ? `${list}\n\nYang ${/mahal/i.test(text) && !/murah/i.test(text) ? 'lebih mahal' : 'lebih murah'}: *${cheaper.title}*${cheaper.area ? ` (${cheaper.area})` : ''} — ${cheaper.priceText}.` : `${list}\n\n${/mahal/i.test(text) ? 'More expensive' : 'Cheaper'}: *${cheaper.title}* — ${cheaper.priceText}.` };
          }
        }
        const answers = [];
        for (const c of set) {
          const ans = await answerCardAttribute({ message: text.replace(/\b(bedanya|perbedaan|banding\w*|masing[-\s]?masing)\b/gi, ' '), card: c, userId: agentUserId, isId });
          answers.push(ans ? ans.reply : (isId ? `Untuk *${c.title}*: ${c.priceText || '-'}${c.address ? `, ${c.address}` : ''}.` : `For *${c.title}*: ${c.priceText || '-'}.`));
        }
        return { verdict: 'follow-up-compare', reply: answers.join('\n\n') };
      }
    }

    // 4) Atribut unit / nego / kondisi (dari kartu yang dipilih atau satu-satunya kartu).
    if (isAttributeQuestion(text)) {
      if (card) {
        const ans = await answerCardAttribute({ message: text, card, userId: agentUserId, isId });
        if (ans) return { verdict: 'post-summary-attribute', reply: ans.reply + assumedNote };
      } else if (shown.length > 1) {
        return {
          verdict: 'post-summary-which-card',
          reply: isId
            ? `Boleh, Kak 😊 Yang ditanyakan unit nomor berapa ya (1-${shown.length})?`
            : `Sure 😊 Which unit number do you mean (1-${shown.length})?`,
        };
      }
    }

    // 5) Budget disebut sesudah summary → catat, jangan mulai interview ulang.
    if (afterSummary && qs && qs.budget && /\d/.test(text) && /\b(budget|dana|anggaran|maksimal|max|sekitar|juta|jt|miliar|m)\b/i.test(text)) {
      return {
        verdict: 'post-summary-budget',
        reply: isId
          ? `Dicatat, Kak 😊 Budget *${qs.budget}* saya teruskan ke agent kami. Ada lagi yang mau ditanyakan?`
          : `Noted 😊 Budget *${qs.budget}* is passed on to our agent. Anything else?`,
      };
    }

    /* M193 — SESUDAH SUMMARY, TIDAK PERNAH RESET KE "HALO! sewa atau beli?".
     * Simulasi: "Ada taman?" sesudah summary jatuh ke Phase 0 (riwayat dipotong
     * di summary) dan bot menyapa seperti customer baru — kehilangan konteks
...
function applyGateCLengthCap(result, { history = [], userMessage = '', agentName = '' } = {}) {
  try {
    ...
    // Konsen versi lama (sesi yang masih berjalan saat rilis) — baca jawabannya.
    ...
    /* M198 (13 Sep 2026) — aturan pemilik proyek: pada pesan customer ke-10-12
     * TANPA pergantian kota/transaksi/tipe, AI LANGSUNG memberikan summary
     * (tidak lagi bertanya "boleh saya lanjut atau ringkas?"). Balasan giliran
     * ini tetap dikirim, summary menyusul di pesan yang sama. Pergantian
     * memulai hitungan dari 1 lagi (countCustomerTurns, dipakai juga oleh
     * fakta turn-count prompt platform). */
    let sinceChange = exchangeCount;
    try {
      const qs = extractQualificationState(history, userMessage) || {};
      sinceChange = countCustomerTurns(history, userMessage, qs).turnsSinceChange;
    } catch (_) { /* fail-open: pakai hitungan sesi */ }
    ...
    // M204: balasan penutup (closing_without_cards/slots) tidak ditempeli pertanyaan slot — customer sudah pamit.
    const isClosingReply = /closing_without/.test(String(result.provider || ''));
    if (!isClosingReply && exchangeCount >= GATE_C_SOFT_CAP && exchangeCount < GATE_C_HARD_CAP) {
      try {
        const qs = extractQualificationState(history, userMessage) || {};
    ...
    if (!isClosingReply && exchangeCount >= GATE_C_SOFT_CAP && sinceChange >= GATE_C_SOFT_CAP && !(awaitingSlotAnswer && exchangeCount < GATE_C_HARD_CAP)) {   // M204: penutup tanpa kartu tidak ditempeli summary
