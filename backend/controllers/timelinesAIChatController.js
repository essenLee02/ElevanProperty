...
async function handleDebouncedBatch({ combinedMessage, sender, name, normSender, source, agent, messageId }) {
  ...
  try {
    ...
    if (cmdReply) {
      await sendViaTimelinesAI(sender, cmdReply);
      console.log(`[TimelinesAI] ⚙️  Perintah agent: catalog_summary di-update oleh ${agent.name}`);
  ...
  try {
    ...
    if (await isAiDisabledForCustomer({ agentUserId: agent.user_id, phone: sender })) {
      if (isTerminalActive('TIMELINESAI')) {
        console.log(`[TIMELINESAI] 🤖⛔ ai_response=OFF — AI diam untuk ${maskPhone(sender)} (agent takeover)`);
      }
      return; // agent takeover — AI tidak membalas
    }
  } catch (offErr) {
    console.warn('[TIMELINESAI] ai_response gate check failed (fail-open):', offErr.message);
  }
