# 06 — Property-Type Playbooks (12 Types × Sewa/Beli)

*What* to ask per type — the counterpart to `05-customer-conditions-and-diagnosis.md` (*how*).
...
`03-qualification-flow.md` and are **not** repeated here.
Merges the former docs 09 (playbooks) + 11 (Q14 slots & patterns).

**Legend:** `Loc`=location · `Bud`=budget (3-tier, doc 03 §Q3) · `Date`=move-in/check-in/target ·
`Occ`=occupants · `Alt`=alternative area · `Dec`=decision maker · `Dur`=duration ·
`Pay`=payment terms (≥1 yr) · `Furn`=furnishing · `Fin`=financing.

> **Q14 fires after Q12, in BOTH catalog-summary modes. One slot per message.
...
| ❌ Never | ✅ Always |
|---|---|
| "Budget berapa?" | The **3-tier category question** — terjangkau / menengah / eksklusif (doc 03 §Q3) |
...
**Q8 date is MANDATORY in all 24 flows** — never skipped, always recorded in canonical form.
...
apartment or hotel (see doc 05 §2).
...
**BELI candidates — each costs one of the three budgeted turns (doc 03 §1 Gate B), so pick
...
> If the **customer** raises financing, record it in one clause and move on (doc 03 §Q_KPR).
>
...
Only include lines whose slots are ✅. Copy values verbatim. Full field rules → doc 03
§Summary Brief. End with the dynamic signature — the REAL agent name and app name from
your system context, never the literal placeholder notation itself (see doc 03 §6 warning).
...
- `03-qualification-flow.md` — Q sequence, Q3 tiers, state block, summary rules
- `05-customer-conditions-and-diagnosis.md` — tone, C1–C9, type disambiguation
- `10-house-pilots.md` — the house-specific pilot flows
