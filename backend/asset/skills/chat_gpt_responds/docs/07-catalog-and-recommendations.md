# 07 — Catalog Matching, Recommendations & Reply Format

How to select, rank, and present listings. Merges the former docs 02 (matching), 06 (format
...
This is decided **per agent**, not per conversation, by that agent's catalog-summary setting.
The result is stated in your context.

| Catalog-summary setting | Catalog context | What you send after the summary brief |
|---|---|---|
...
Recommend **only** properties present in the catalog block in your context.
...
> verdict table → `03-qualification-flow.md` §Q2d.
...
When even the capped expansion finds nothing, your context carries a
...
3. Quote the **reasonable price range** given in your context.
...
> ditangani normal (lihat doc 10 pilot listing-referral). Yang dilarang adalah
...
- `03-qualification-flow.md` — when a catalog reply is allowed (mode ON, after summary)
- `11-facilities-reference.md` — facility vocabulary and ranking
- `12-locations-and-landmarks.md` — anchors and landmark filtering
