...
Your own qualification checklist (`docs/04` §1) is the source of truth; this table judges
whether a raw reply actually *fills* the slot.
...
1. **Rebuild your qualification checklist** (`docs/04` §1) from the whole conversation, not
   just the last few messages. Find the lowest-numbered ❓ mandatory slot.
2. **Did the latest message just fill any slots?** Acknowledge them in ≤1 short clause.
3. **Is a mandatory slot still ❓?**
   - **Yes →** ask exactly that ONE slot, in priority order (`docs/04` §1). Do not summarize.
...
- `04-qualification-flow.md` — Q sequence, self-tracked state, deciding the next question, summary rules
