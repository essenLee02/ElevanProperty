...
function canUseOpenRouter() {
  ...
  return cfg.hasApiKey && Boolean(cfg.model);
}
...
async function executeAIProviderWithFallback(taskName, chatGPTFn, claudeFn, qwenFn = null, deepseekFn = null, kimiFn = null, agentAiPrimary = '', openrouterFn = null) {
  ...
  const fns   = { chatgpt: chatGPTFn, claude: claudeFn, qwen: qwenFn, deepseek: deepseekFn, kimi: kimiFn, openrouter: openrouterFn };
  const avail = {
    ...
    openrouter : () => canUseOpenRouter() && !!openrouterFn,
  };
...
async function executeExternalAIFallbackChain(taskName, chatGPTFn, claudeFn, qwenFn = null, deepseekFn = null, kimiFn = null, openrouterFn = null) {
  const fns   = { chatgpt: chatGPTFn, claude: claudeFn, qwen: qwenFn, deepseek: deepseekFn, kimi: kimiFn, openrouter: openrouterFn };
  const avail = {
    chatgpt    : () => canUseChatGPT()    && !!chatGPTFn,
    claude     : () => canUseClaude()     && !!claudeFn,
  ...
  // ⚠️ Urutan DeepSeek→Kimi→Claude→ChatGPT→QWEN adalah kontrak terdokumentasi
  // (SKILL.md §3) — OpenRouter ditambahkan di EKOR, bukan disisipkan, supaya
  // urutan lima provider lama tidak berubah untuk siapa pun yang belum
  // mengatur OPENROUTER_API_KEY.
  for (const providerName of ['deepseek', 'kimi', 'claude', 'chatgpt', 'qwen', 'openrouter']) {
    if (!avail[providerName]()) {
...
async function generateContactReplyWithProviderFallback(contactPayload) {
  return executeAIProviderWithFallback(
    ...
    '',
    () => generateOpenRouterContactReply(contactPayload)
  );
}
...
async function generateChatbotReplyWithProviderFallback(session, history, userMessage, propertyContext = '') {
  return executeAIProviderWithFallback(
    ...
    '',
    () => generateOpenRouterChatbotReply(session, history, userMessage, propertyContext)
  );
}
...
async function generateWhatsappReplyWithProviderFallback(session, history, userMessage, propertyContext = '', extraContext = {}) {
  return executeAIProviderWithFallback(
    ...
    () => generateOpenRouterWhatsappReply(session, history, userMessage, propertyContext, extraContext)
  );
}

// Fallback eksternal saat Private Agent gagal (primary=private):
// urutan: DeepSeek → Kimi → Claude → ChatGPT → QWEN → OpenRouter
