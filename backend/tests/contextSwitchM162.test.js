...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   membuat LLM membantah kode yang sudah diperbaiki. Kalau doc 04 masih menulis
...
console.log('\n── Group 7: skill docs sinkron dengan kode ──');
{
  ...
  for (const folder of FOLDERS) {
    const doc = fs.readFileSync(path.join(root, folder, 'docs', '04-qualification-flow.md'), 'utf8');
  ...
  const a = fs.readFileSync(path.join(root, 'claude_responds',   'docs', '04-qualification-flow.md'));
  const b = fs.readFileSync(path.join(root, 'chat_gpt_responds', 'docs', '04-qualification-flow.md'));
  ok('claude_responds & chat_gpt_responds doc 04 tetap BYTE-IDENTICAL', a.equals(b));
}
